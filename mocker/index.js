...
const randomNumberOfCuisines = 10;
...
const pairings = new Array(randomNumberOfPairings)
  ...
  .filter(
    pairing => pairing.source !== pairing.target,
  );
    ...
    ingredients: Array.from(
      new Set(
        new Array(Math.ceil(Math.random() * 10))
          .fill(undefined)
          .map(() => ingredients[Math.floor(Math.random() * ingredients.length)].id),
      ),
    ),
  }));
...
const proxy = {
  ...
  'GET /cuisines': (req, res) => {
    setTimeout(() => res.json(cuisines.map(cuisine => ({
      id: cuisine.id,
      name: cuisine.name,
